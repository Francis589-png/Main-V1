...
export function watchMessages(chatId, callback) {
  ...
  (async () => { try {
    ...
    unsubscribe = onSnapshot(q, snapshot => {
      ...
      const uid = auth.currentUser?.uid; if (uid) messages.filter(message => message.senderId !== uid && !message.deliveredAt?.[uid]).forEach(message => markDelivered(chatId, message.id).catch(() => {}));
...
export async function sendMessage(chatId, text, options = {}) {
  const fs = requireFirestore(); const me = auth.currentUser; if (!me) throw new Error('You must be signed in.'); const cleanText = String(text || '').trim(); if (!cleanText) return null; if (!(await getDoc(doc(fs, 'conversations', chatId, 'members', me.uid))).exists()) throw new Error('You are not a member of this conversation.');
