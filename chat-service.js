...
export async function createGroup(name, members) {
  ...
  const memberMap = Object.fromEntries(unique.map(uid => [uid, true]));
  const writes = {
    [`chats/${chatId}`]: { chatId, type: 'group', name: cleanName, createdBy: me.uid, createdAt: serverTimestamp(), updatedAt: serverTimestamp(), members: memberMap }
  };
...
export function watchMessages(chatId, callback) {
  return onValue(ref(db, `messages/${chatId}`), s => {
    const value = s.val() || {};
...
export function watchTyping(chatId, otherUid, callback) {
  return onValue(ref(db, `typing/${chatId}/${otherUid}`), s => callback(Boolean(s.val())));
}
...
export async function sendMessage(chatId, text) {
  ...
  const recipientIds = Object.keys(chat.members).filter(uid => uid !== auth.currentUser.uid);
  const messageRef = push(ref(db, `messages/${chatId}`));
  const createdAt = Date.now();
  const message = { senderId: auth.currentUser.uid, text: cleanText, type: 'text', createdAt, serverCreatedAt: serverTimestamp(), readBy: { [auth.currentUser.uid]: true } };
  const writes = {
    [`messages/${chatId}/${messageRef.key}`]: message,
    [`chats/${chatId}/lastMessage`]: cleanText,
    [`chats/${chatId}/lastSenderId`]: auth.currentUser.uid,
    [`chats/${chatId}/updatedAt`]: serverTimestamp()
  };

  if (chat.type === 'group') {
    for (const uid of Object.keys(chat.members)) {
      const unread = uid === auth.currentUser.uid ? 0 : (Number((await get(ref(db, `userChats/${uid}/${chatId}/unreadCount`))).val()) || 0) + 1;
      writes[`userChats/${uid}/${chatId}`] = { chatId, type: 'group', groupName: chat.name, otherName: chat.name, lastMessage: cleanText, lastSenderId: auth.currentUser.uid, updatedAt: createdAt, unreadCount: unread };
    }
  } else {
    const otherUid = recipientIds[0];
    const profile = (await get(ref(db, `publicProfiles/${otherUid}`))).val() || {};
    const unread = (Number((await get(ref(db, `userChats/${otherUid}/${chatId}/unreadCount`))).val()) || 0) + 1;
    ...
    writes[`userChats/${otherUid}/${chatId}`] = { chatId, otherUid: auth.currentUser.uid, otherName: auth.currentUser.displayName || 'Main user', lastMessage: cleanText, lastSenderId: auth.currentUser.uid, updatedAt: createdAt, unreadCount: unread };
  }
  await update(ref(db), writes);
}
