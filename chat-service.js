...
export async function createGroup(name, members) {
  ...
  if (!cleanName) throw new Error('Enter a group name.'); if (unique.length < 2) throw new Error('A group needs at least one other person.'); const conversationRef = doc(collection(fs, 'conversations'));
...
export function watchMessages(chatId, callback) {
  ...
  (async () => { try {
    ...
    unsubscribe = onSnapshot(q, snapshot => {
      const messages = normalizeMessages(snapshot); window.__mainActiveMessages = messages; window.__mainActiveChatId = chatId; callback(messages);
      const uid = auth.currentUser?.uid; if (uid) messages.filter(message => message.senderId !== uid && !message.deliveredAt?.[uid]).slice(-20).forEach(message => markDelivered(chatId, message.id).catch(() => {}));
...
export async function sendMessage(chatId, text, options = {}) {
  const fs = requireFirestore(); const me = auth.currentUser; if (!me) throw new Error('You must be signed in.'); const type = options.type || 'text'; const cleanText = String(text || '').trim(); if (!cleanText && !options.mediaId && type !== 'audio' && type !== 'voice') return null; if (!(await getDoc(doc(fs, 'conversations', chatId, 'members', me.uid))).exists()) throw new Error('You are not a member of this conversation.');
  const messageRef = doc(collection(fs, 'conversations', chatId, 'messages')); await setDoc(messageRef, { senderId: me.uid, type, text: cleanText, replyTo: options.replyTo || null, forwardedFrom: options.forwardedFrom || null, mediaId: options.mediaId || null, createdAt: firestoreServerTimestamp(), deliveredAt: { [me.uid]: firestoreServerTimestamp() }, readBy: { [me.uid]: firestoreServerTimestamp() }, reactions: {}, starredBy: {} }); return messageRef.id;
}
