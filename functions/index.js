...
export const indexNewMessage = onDocumentCreated(
  ...
  async event => {
    ...
    const members = await db.collection(`conversations/${conversationId}/members`).get();
    const batch = db.batch();
    const preview = message.text || `[${message.type || 'message'}]`;
    for (const member of members.docs) {
      const uid = member.id;
      const indexRef = db.doc(`userConversations/${uid}/items/${conversationId}`);
      const data = {
        chatId: conversationId,
        type: conversation.data().type,
        unreadCount: uid === message.senderId ? 0 : FieldValue.increment(1),
        lastMessage: preview,
        lastSenderId: message.senderId,
        updatedAt: FieldValue.serverTimestamp()
      };
      if (conversation.data().type === 'group') {
        data.groupName = conversation.data().name || 'Group';
        data.otherName = data.groupName;
      }
      batch.set(indexRef, data, { merge: true });
    }
    await batch.commit();
  }
);
